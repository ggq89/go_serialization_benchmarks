...
// DecodeMsg implements msgp.Decodable
func (z *NoTimeNoStringNoFloatA) DecodeMsg(dc *msgp.Reader) (err error) {
	var field []byte
	_ = field
	var zb0001 uint32
	zb0001, err = dc.ReadMapHeader()
	...
	for zb0001 > 0 {
		...
		case "BirthDay":
			z.BirthDay, err = dc.ReadUint64()
			if err != nil {
				err = msgp.WrapError(err, "BirthDay")
				return
			}
		case "Phone":
			z.Phone, err = dc.ReadBytes(z.Phone)
			if err != nil {
				err = msgp.WrapError(err, "Phone")
				return
			}
		case "Siblings":
			z.Siblings, err = dc.ReadUint32()
			if err != nil {
				err = msgp.WrapError(err, "Siblings")
				return
			}
...
// EncodeMsg implements msgp.Encodable
func (z *NoTimeNoStringNoFloatA) EncodeMsg(en *msgp.Writer) (err error) {
	// map header, size 6
	// write "Name"
	err = en.Append(0x86, 0xa4, 0x4e, 0x61, 0x6d, 0x65)
	if err != nil {
